...
class FreezeFrame:
    ...
    def getTrainingCols(self):
        return pd.MultiIndex.from_arrays([['Animal ID', ' ', ' '] + ['CS+']*6 + ['CS-']*6 + ['US']*6 + [' '] + ['Post-CS+ ITI']*6  + ['Post-CS- ITI']*5 + ['Post-US ITI']*6 + [' ']*2,
                                          ['', 'Threshold', 'Pre-CS'] + [str(i) for i in range(1, 6)] + ['Mean CS+'] + [str(i) for i in range(1, 6)] + ['Mean CS-'] + [str(i) for i in range(1, 6)] + ['Mean US'] + ['D.I.'] + [str(i) for i in range(1, 6)] + ['Mean Post-CS+ ITI'] + [str(i) for i in range(1, 5)] + ['Mean Post-CS- ITI'] + [str(i) for i in range(1, 6)] + ['Post-US ITI'] + ['Mean ITI', 'Post-CS']])
    ...
    def process_data(self, ff_df, timestamps, exp):
        ...
        if exp == 'Training':
            us_start, us_end = self.extract_timestamps(timestamps, 'US')
        ...
        if exp == 'Training':
            post_us_start, post_us_end = self.extract_timestamps(timestamps, 'Post-US') # extract the start and end timestamps for Post-US
        ...
        for animal_id in ff_df.iloc[1:, 0]: # for each animal ID
            ...
            if exp == 'Training':
                us = [self.get_ff_avg(animal_id, start, end, ff_df) for start, end in zip(us_start, us_end)]
                mean_us = round(np.mean(us), 2)

                post_us = [self.get_ff_avg(animal_id, start, end, ff_df) for start, end in zip(post_us_start, post_us_end)]
                post_us_iti = round(np.mean(post_us), 2)

                mean_iti = round(np.sum([*post_cs_plus_iti, *post_cs_minus_iti, *post_us]) / len([*post_cs_plus_iti, *post_cs_minus_iti, *post_us]), 2) # calculate the mean of the ITI data

                data = [animal_id.split()[-1], threshold, pre_cs[0], *cs_plus, mean_cs_plus, *cs_minus, mean_cs_minus, *us, mean_us, di, *post_cs_plus_iti, mean_post_cs_plus_iti, *post_cs_minus_iti, mean_post_cs_minus_iti, *post_us, post_us_iti, mean_iti, post_cs[-1]]
            else:
